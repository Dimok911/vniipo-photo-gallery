...
async function setup(page) {
  ...
  await page.evaluate(async () => {
    ...
    window.switcher = VniipoPhotoGallery.createFullscreenSwitcher({
      ...
      onTouchPagingPosition(value) { frames.push({...value, left:track.scrollLeft}); },
    ...
    track.addEventListener('touchstart', event => {
      ...
      pinching = true;
      switcher.goTo(Math.round(track.scrollLeft / track.clientWidth),'instant');
    });
...
test('released swipe accepts a NEW pinch while still settling, with no late position or bitmap reset', async ({page}) => {
  await setup(page);
  const policy = await page.evaluate(() => {
    const style = getComputedStyle(track);
    return [style.overflowX,style.touchAction,style.scrollSnapType,style.scrollBehavior];
  });
  expect(policy).toEqual(['hidden','none','none','auto']);
  await page.evaluate(() => touch('touchstart',[[350,200]]));
  await page.waitForTimeout(30);
  await page.evaluate(() => { touch('touchmove',[[100,200]]); touch('touchend',[]); });
  await page.waitForTimeout(30);
  expect(await page.evaluate(() => switcher.isSettling && track.scrollLeft > track.clientWidth/2 && track.scrollLeft < track.clientWidth)).toBe(true);
  const takeover = await page.evaluate(() => {
    touch('touchstart',[[130,200],[230,200]]);
    touch('touchmove',[[80,200],[280,200]]);
    return {scale,settling:switcher.isSettling,left:track.scrollLeft, callbacks:starts.at(-1)};
  });
  ...
  expect(await page.evaluate(() => ({scale,left:track.scrollLeft,same:images.every((image,index)=>image===document.images[index]),
    transform:slides[1].firstElementChild.style.transform}))).toEqual({scale:2,left:takeover.left,same:true,transform:'scale(2)'});
});

test('smooth goTo can be interrupted by a new swipe and edge return can be interrupted by pinch', async ({page}) => {
  await setup(page);
  ...
  expect(await page.evaluate(() => parseFloat(slides[2].firstElementChild.style.translate))).toBe(-44);
...
test('reduced motion settles immediately and destroy restores consumer scroll styles', async ({page}) => {
